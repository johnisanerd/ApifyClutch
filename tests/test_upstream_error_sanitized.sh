...
PYTEST="${TESTS_DIR}/test_upstream_error_sanitized.py"
...
if command -v uv >/dev/null 2>&1 && uv run --directory "${ACTOR_DIR}" python -c 'import curl_cffi' >/dev/null 2>&1; then
  ...
  if [ -z "${PY}" ]; then
    echo "FAIL: test_upstream_error_sanitized could not run - no Python env with curl_cffi. NOT skipping." >&2
    exit 1
  fi
  "${PY}" "${PYTEST}"; rc=$?
fi

if [ "${rc}" -ne 0 ]; then echo "FAIL: test_upstream_error_sanitized (exit ${rc})." >&2; exit 1; fi
exit 0
